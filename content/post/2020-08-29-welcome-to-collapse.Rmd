...
[*collapse*](https://sebkrantz.github.io/collapse/) is a C/C++ based package for data transformation and statistical computing in R. It was first released on CRAN end of March 2020. The current version 1.3.1 is a mature package secured by > 7700 unit tests. *collapse* has 2 main aims: 
...
EuStockMarkets[, .c(DAX, SMI)] %>% fdiff(0:1 * frequency(.), 1:2) %>% 
...
# Weighted demeaning
iris %>% fgroup_by(Species) %>% num_vars %>% fwithin(wt) %>% head(3)  
...
collap(wlddev, ~ iso3c + decade, fmean, catFUN = fmode) %>% head(3)

# Same as a piped call.. 
wlddev %>% fgroup_by(iso3c, decade) %>% collapg(fmean, fmode) %>% head(3)
...
collap(wlddev, PCGDP + LIFEEX ~ region + income, list(fmean, fsd, fmin, fmax), return = "long") %>% head(3)

```

The default (`keep.col.order = TRUE`) ensures that the data remains in the same order, and, when working with *Fast Statistical Functions*, all column attributes are preserved. It is also possible to provide fully customized calls:
